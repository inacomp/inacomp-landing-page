              ...
              <div className="grid gap-4 sm:grid-cols-[1.2fr_0.8fr]">
                <div className="surface-panel p-4">
                  <div className="mb-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Badge tone="neutral" className="text-[10px]">Live</Badge>
                    </div>
                  </div>
                  <div className="space-y-2">
                    {[
                      ...
                      <div key={idx} className="flex items-center justify-between rounded-xl bg-white/60 p-2.5 px-4 border border-white/40">
                         <div className="flex items-center gap-3">
                            <span className="font-mono text-xs text-muted w-4">{idx + 1}</span>
                ...
                <div className="surface-panel p-4">
                  ...
                  <div className="mt-4 space-y-2">
                    {[
                      { status: "Accepted", color: "text-emerald-600", bg: "bg-emerald-50" },
                      { status: "Wrong Ans", color: "text-rose-600", bg: "bg-rose-50" },
                      { status: "Time Limit", color: "text-amber-600", bg: "bg-amber-50" },
                      { status: "Accepted", color: "text-emerald-600", bg: "bg-emerald-50" }
                    ].map((item, idx) => (
                      <div key={idx} className={`flex items-center gap-2 rounded-lg ${item.bg} p-2 px-3 border border-white/40`}>
                        <div className={`h-1.5 w-1.5 rounded-full ${item.status === 'Accepted' ? 'bg-emerald-500' : 'bg-current'}`} />
